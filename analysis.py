...
import os
import warnings
import matplotlib.ticker as mtick  # 新增导入：用于格式化百分比坐标轴
...
os.makedirs('analysis_results/maps', exist_ok=True)
os.makedirs('analysis_results/trends', exist_ok=True)

def analyze_taxi_data_weekly(file_path):
    print("="*50)
    print("网约车业务数据分析工具 (含热门地点与时长分布)")
    ...
    try:
        ...
        plt.title('每日订单量变化趋势', fontsize=14)
        plt.xlabel('')
        plt.ylabel('订单量')
        plt.grid(axis='y', linestyle='--', alpha=0.5)
        plt.savefig('analysis_results/trends/weekly_trend.png', dpi=300)
        plt.close()

        # B. 运营节奏热力图 (仅工作日)
        print("  - 生成运营节奏热力图 (仅工作日)...")
        ...
        plt.title('运营节奏热力图 (周一至周五)', fontsize=14)
        plt.ylabel('小时 (0-23)')
        plt.xlabel('工作日')
        plt.savefig('analysis_results/trends/weekly_heatmap.png', dpi=300)
        plt.close()

        # C. 订单时间（行程时长）从长到短排序图 (新增需求)
        print("  - 生成订单行程时长降序分布图...")
        # 提取时长并降序排列
        ...
        # 优化坐标轴显示范围
        plt.xlim(0, 100)
        # 截断极少数的极端异常值（取 99.9% 分位数作为上限），防止主图形被严重压扁
        ...
        plt.title('订单行程时长分布 (从长到短降序排列)', fontsize=14, pad=15)
        plt.xlabel('订单比例 (%)', fontsize=12)
        plt.ylabel('行程时长 (分钟)', fontsize=12)
        plt.legend(fontsize=12)
        plt.grid(axis='both', linestyle='--', alpha=0.4)
        plt.tight_layout()
        plt.savefig('analysis_results/trends/duration_descending.png', dpi=300)
        plt.close()
        ...
        if 'dep_area' in df.columns and 'dest_area' in df.columns:
            ...
            axes[0].set_title('Top 5 热门上车点')
            axes[0].set_xlabel('订单量')

            # 下车点图
            sns.barplot(x=top5_dest.values, y=top5_dest.index, ax=axes[1], palette='magma')
            axes[1].set_title('Top 5 热门下车点')
            axes[1].set_xlabel('订单量')

            plt.tight_layout()
            plt.savefig('analysis_results/trends/top_locations.png', dpi=300)
            plt.close()
        ...
        if len(clean_df) > 0:
            ...
            dep_map.save('analysis_results/maps/weekly_heatmap.html')
...
{top_dest_str}
        ...
        最热时段: {peak_hour}:00 - {peak_hour+1}:00 (共 {peak_hour_vol} 单)
        ...
        print(report)
        with open('analysis_results/report.txt', 'w', encoding='utf-8') as f:
            f.write(report)

        print("\n✅ 分析完成！请查看 analysis_results 文件夹，新增了 duration_descending.png 图表。")
        return True
...
if __name__ == "__main__":
    input_file = "data/finaldata.csv"
    if os.path.exists(input_file):
        analyze_taxi_data_weekly(input_file)
    else:
